...
async function sendGeneratedReply({
  ...
  const guildLore = loreGraphService.getRelevantLore(message.guild.id, 6);
  const affinity = affinityService.getUserAffinity(message.guild.id, message.author.id);
  ...
  const result = await withTimeout(
    responseGenerator.generateChatReply({
  ...
  const sent = await sendMultiBubble({
    channel: message.channel,
    replyToMessage: message,
    bubbles,
    burstDelayMs: config.bot.burstDelayMs
  });
  ...
  recordBotReplyMessage({
    ...
    sent,
    content,
    personality: result.personality?.id || 'chill'
  });

  // Update affinity
  affinityService.registerInteraction({
    guildId: message.guild.id,
    userId: message.author.id,
    type: repliedToBot ? 'reply' : mentioned ? 'mention' : 'ambient'
  });
...
async function sendTargetGremlinReply({ message, action, recentMessages, allowReact }) {
  ...
  try {
    content = await withTimeout(
      responseGenerator.generateGremlinReply({
        botName: message.client.user.username,
        guildName: message.guild.name,
        channelName: message.channel.name || 'chat',
...
async function sendAutonomousChannelMessage({ message, content, personality, withPoll = false, topic = '' }) {
  await safeTyping(message.channel);
  await sleep(calculateTypingDelay(content));

  let sent = null;
  if (withPoll && config.bot.enableHotTakePolls && topic) {
    const row = createHotTakeActionRow(topic);
    sent = await message.channel.send({
      content,
      components: [row]
    }).catch(() => null);
  } else {
    sent = await safeSend(message.channel, content);
  }
...
async function execute(message) {
  ...
  // Active Hot Take / Debate Evaluation
  ...
  // Autonomous Hot Take Initiation
  if (
    !forced &&
    !activeHotTake &&
  ...
  // Ambient Conversation Decision & Delivery
  if (!forced) {
    ...
    if (activeHotTake && hotTakeRelated) {
      ...
      if (!decision.shouldReply) {
        if (decision.score > 0 && chance(config.bot.reactionChance)) {
          ...
          const reacted = await safeReact(message, emoji);
          if (reacted) {
            cooldowns.markTalk(message.guild.id, message.channel.id, message.author.id);
            repositories.recordBotReaction({
              guildId: message.guild.id,
              userId: message.author.id,
              channelId: message.channel.id
