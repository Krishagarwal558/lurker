...
function classifyMemory(text) {
  ...
  if (/(inside joke|joke between us|server joke|running bit|running joke)/i.test(lower)) return 'inside_joke';
  if (/(meme|side quest|lore|legendary moment)/i.test(lower)) return 'running_meme';
  if (/(favorite|favourite|fav).*(game|valorant|minecraft|gta|csgo|fortnite|roblox)|\b(valorant|minecraft|league)\b/i.test(lower)) {
    return 'favorite_game';
  }
  if (/(anime|movie|coding|food|gym|college|exam|sleep|topic|music|song|band)/i.test(lower)) {
...
function isAskingWhatRemember(content) {
  return /\bwhat do you remember\b/i.test(content) ||
    ...
    /\bwhat do you know about me\b/i.test(content);
}
...
function formatMemoryList(memories) {
  if (!memories || !memories.length) return 'nothing saved yet';
  return memories
