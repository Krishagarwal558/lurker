...
const OPENERS = [
  ...
  'unpopular opinion rn'
];

const AGREEMENT = /\b(facts|true|agreed|agree|same|valid|real|yes|yeah|yep|fr|exactly|correct|based|fair|w take|so true|spitting)\b/i;
const DISAGREEMENT = /\b(no|nah|wrong|cap|disagree|bad take|l take|not true|never|false|trash take|terrible take|clown take)\b/i;
const SHORT_RELATED = /\b(coffee|tea|pizza|windows|linux|minecraft|valorant|anime|movie|food|sleep|gym|exam|college|coding|ketchup|fries|rust|tabs|spaces)\b/i;
...
function voteOnHotTake({ guildId, channelId, topic, userId, voteType }) {
  return repositories.castHotTakeVote({ guildId, channelId, topic, userId, voteType });
}
