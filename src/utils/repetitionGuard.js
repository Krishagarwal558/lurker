...
function recentBotMessages(messages, lookback = config.bot?.duplicateLookbackMessages || 12) {
  return messages
...
function freshFallback(recentMessages) {
  ...
  return pick(options.length ? options : config.bot?.freshFallbackReplies || ['real tbh', 'fair enough']);
}
