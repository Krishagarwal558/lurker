...
function isOwnerOrAdmin(messageOrMember, authorId) {
  const member = messageOrMember.member || messageOrMember;
  const userId = authorId || messageOrMember.author?.id || messageOrMember.user?.id;
  return isOwner(userId) || isAdministrator(member);
}
...
function formatChannelList(guild, rows) {
...
const blacklist = {
  ...
  async execute({ message, args }) {
    ...
    if (action === 'list') {
      ...
      await safeReply(message, `blacklisted channels: ${formatChannelList(message.guild, rows)}`);
      return;
    }
...
const whitelist = {
  ...
  async execute({ message, args }) {
    ...
    if (action === 'list') {
      ...
      await safeReply(message, `whitelisted channels: ${formatChannelList(message.guild, rows)}`);
      return;
    }
